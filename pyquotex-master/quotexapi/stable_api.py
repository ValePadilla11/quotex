    ...
    async def check_win(self, id_number):
        ...
        while True:
            if time.time() - start_time > timeout:
                print("Tiempo de espera excedido al verificar el resultado.")
                return 'unknown'
            try:
                ...
                print(f"Error al obtener datos de la lista: {e}")
            await asyncio.sleep(0.1)
    # Resto del código

        self.api.listinfodata.delete(id_number)
        win_result = listinfodata_dict["win"]
        print(f"Tipo de win_result: {type(win_result)}, valor: {win_result}")
        if asyncio.iscoroutine(win_result):
            win_result = await win_result
        return win_result
